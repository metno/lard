...
(**NOTE:** the sine wave product is used only as a demonstration of the API, it doesn't make use of
any observations at all)

Get a product:

```text
$ curl -X POST 'localhost:3000/product/sinewave' -d '{"from_time":1,"to_time":20,"time_resolution":1,"min_value":-10,"max_value":10,"frequency":0.05}' -w '\n'
