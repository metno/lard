...
// Define some constansts that are used in the calculations of humidity-related products.
const C: f64 = 6.10780; // [hPa]
const A_POS: f64 = 17.08085; // [rate] or [deg.C / deg.C]
const A_NEG: f64 = 17.84362; // [rate] or [deg.C / deg.C]
const B_POS: f64 = 234.175; // [deg.C]
const B_NEG: f64 = 245.425; // [deg.C]
...
///
/// Approximations: Tetens or August-Roche-Magnus equation... the form is the same, but coefficients differ based on temperature range.
///
...
fn calculate_saturation_vapor_pressure(air_temperature: f64) -> f64 {
    if air_temperature <= 0.0 {
        return C * (A_NEG * air_temperature / (B_NEG + air_temperature)).exp();
    }
    C * (A_POS * air_temperature / (B_POS + air_temperature)).exp()
}
...
pub fn calculate_dew_point_temperature(
    ...
    // TODO: Should we set a condition that dew_point_temperature cannot be greater than air_temperature???
    if air_temperature <= 0.0 {
        return B_NEG * (water_vapor_partial_pressure / C).ln()
            / (A_NEG - (water_vapor_partial_pressure / C).ln());
    }
    B_POS * (water_vapor_partial_pressure / C).ln()
        / (A_POS - (water_vapor_partial_pressure / C).ln())
}
...
pub fn water_vapor_partial_pressure_in_air(air_temperature: f64, relative_humidity: f64) -> f64 {
...
pub fn dew_point_temperature(air_temperature: f64, relative_humidity: f64) -> f64 {
    let saturation_vapor_pressure = calculate_saturation_vapor_pressure(air_temperature);
    let water_vapor_partial_pressure =
        calculate_water_vapor_partial_pressure(saturation_vapor_pressure, relative_humidity);
    calculate_dew_point_temperature(water_vapor_partial_pressure, air_temperature)
}
...
pub fn specific_humidity(
    ...
    let humidity_mixing_ratio =
        calculate_humidity_mixing_ratio(water_vapor_partial_pressure, surface_air_pressure);
    calculate_specific_humidity(humidity_mixing_ratio)
}
